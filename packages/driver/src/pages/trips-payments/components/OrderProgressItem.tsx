...
const OrderProgressItem: React.FC<IProps> = ({
  ...
  return (
    <div className="flex flex-row gap-1 ml-0.5">
      ...
      <div className="flex flex-col justify-between font-normal text-xs pl-2 gap-3">
        <Label title={pickAddress ?? "Pick-Up Address"} primary />
        <Label title={dropAddress ?? "Drop-Off Address"} primary />
      </div>
    </div>
  );
};
